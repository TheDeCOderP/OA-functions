...
add_action( 'woocommerce_cart_calculate_fees', 'add_extra_charge_based_on_weight' );
function add_extra_charge_based_on_weight( $cart ) {
    if ( is_admin() && ! defined( 'DOING_AJAX' ) ) {
        return;
    }
    $extra_charge = 0.49; // The extra charge per kg
    $max_weight = 30; // The maximum weight before extra charge applies
    $cart_weight = $cart->get_cart_contents_weight();
    if ( $cart_weight > $max_weight ) {
        $extra_weight = $cart_weight - $max_weight;
        $extra_charge_total = $extra_weight * $extra_charge;
        $cart->add_fee( 'Extra Weight Charge', $extra_charge_total, false );
    }
}
